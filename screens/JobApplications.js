// screens/JobApplications.js (New - Employer Applications Viewer)
...
import { NotificationIcon, UserIcon, ArrowRightIcon, StarIcon } from '../components/Icons';
...
export default function JobApplications({ navigation }) {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      setError(null);
      // Assuming jobAPI.getApplications() returns { applications: [{ jobId, candidate: { fullName, score, stack, ... }, coverLetter, appliedAt, status }] }
  ...
  const handleViewCandidate = (candidate) => {
    navigation.navigate('CandidateProfile', { candidate });  // Assumes CandidateProfile screen exists
  };

  const handleAction = (action, applicationId) => {
    // TODO: Implement accept/reject via jobAPI.updateApplicationStatus(applicationId, { status: action })
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <TouchableOpacity onPress={() => Alert.alert('Refresh', 'Pull to refresh or tap to manual reload')} style={styles.refreshButton}>
